...
class KSTradeApi:
    def __init__(self, access_token=None, userid=None, consumer_key=None, ip=None, app_id="", host=None,
        ...
        for host in hosts:
            configuration = ks_api_client.Configuration(host=host, userid=userid, access_token=access_token,
                                                        consumer_key=consumer_key, consumer_secret=consumer_secret, ip=ip, app_id=app_id,
                                                        proxy_user=proxy_user, proxy_pass=proxy_pass, proxy_url=proxy_url)
            ...
            # no need to initiate session if session_token is already available: just exit from loop
            if session_token: break

            try:
                session_init_res = ks_api_client.SessionApi(self.api_client).session_init(self.userid, self.consumer_key, self.ip, self.app_id)
                ...
                if self.host != session_init['redirect']['host']:
                    ...
                    self.api_client = ks_api_client.ApiClient(configuration)
                    session_init = ks_api_client.SessionApi(self.api_client).session_init(self.userid, self.consumer_key, self.ip, self.app_id)
                break
    ...
    def order_report(self, order_id=None, is_fno=""):
        if not 'session_token' in self.__dict__:
            raise ApiValueError("Please invoke 'session_2fa' function first")
        if order_id is None:
            order_report = ks_api_client.ReportsApi(self.api_client).get_order_reports(self.consumer_key, self.session_token)
        elif is_fno and is_fno in ("Y", "N", "y", "n"):
            is_fno = is_fno.capitalize()
            order_report = ks_api_client.ReportsApi(self.api_client).get_fno_order_detail_by_order_id(order_id, self.consumer_key, self.session_token, is_fno)
        else:
            order_report = ks_api_client.ReportsApi(self.api_client).get_order_report_by_order_id(order_id, self.consumer_key, self.session_token)
        return order_report
    ...
    def trade_report(self, order_id=None, is_fno=""):
        ...
        if order_id is None:
            ...
            is_fno = is_fno.capitalize()
            trade_report = ks_api_client.ReportsApi(self.api_client).get_fno_trade_report_by_order_id(order_id, self.consumer_key, self.session_token, is_fno)
        else:
            trade_report = ks_api_client.ReportsApi(self.api_client).get_trade_report_by_order_id(order_id, self.consumer_key, self.session_token)
        return trade_report
    ...
    def quote(self, instrument_token, quote_type=None):
        ...
        if quote_type is None:
            quote = ks_api_client.QuoteApi(self.api_client).get_instruments_details(self.consumer_key, self.session_token, instrument_token)
        elif quote_type == 'LTP':
            quote = ks_api_client.QuoteApi(self.api_client).get_ltp_quote(self.consumer_key, self.session_token, instrument_token)
        elif quote_type == 'DEPTH':
            quote = ks_api_client.QuoteApi(self.api_client).get_market_details_quote(self.consumer_key, self.session_token, instrument_token)
        elif quote_type == 'OHLC':
            quote = ks_api_client.QuoteApi(self.api_client).get_ohlc_quote(self.consumer_key, self.session_token, instrument_token)
        else:
